...
English version: [Development Timeline and Resource Assessment](./TopoLab_development_timeline_and_resources.md)

阅读提示：本文保留早期时间和资源估算；完整 v2 旗舰项目的交付还需通过
[v2 路线图](./TopoLab_v2_开发路线图.md)规定的 ML 端到端加速 Gate，
其研发周期不能由下文旧估算保证。

相关文档：[美研申请匹配度评估](./TopoLab_美研申请匹配度评估.md) · [Numerical Core 重写策略](./TopoLab_数值核心重写策略.md)
