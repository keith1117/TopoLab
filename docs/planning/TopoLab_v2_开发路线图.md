...
敏感度加权候选组未解决收敛、柔度质量及细化耗时瓶颈。下一独立切片为
**B2.6**：先冻结新的开发证据边界，再针对已测得的质量与求解轨迹差距实施
一个版本化干预。在开发用原型证明同等质量的端到端收益之前，不进入 B3
的新 final cohort 预注册。
...
12. **B2.6 有针对性的开发干预**：下一独立切片；冻结新的开发证据边界与单一
    版本化目标，针对求解细化耗时和质量问题再开展拟合，保留 B2.5 的负面结论。
13. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
    边界，再分成独立可审查的切片实现和审计数据。
14. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
15. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
    B4 通过后执行。
16. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
