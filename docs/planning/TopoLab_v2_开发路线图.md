...
B2.5、B2.6 的固定 learned 原型可行性 Gate 均未通过；
...
下一独立切片为 **B2.7**：预先固定一个针对方向相关预测/质量和细化耗时差距的
修复方案，再用新的、互不重叠的开发病例筛选。在开发用原型证明同等质量的
...
13. **B2.7 有针对性的开发修复**：下一独立切片；先冻结一项针对方向相关质量
    与细化耗时差距的干预，再使用全新且不重叠的开发病例。保留 B2.6 的负面
    结果以及最终证据边界。
14. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
    边界，再分成独立可审查的切片实现和审计数据。
15. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
16. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
    B4 通过后执行。
17. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
