...
B2.5–B2.8 的固定 learned 原型可行性 Gate 均未通过；
...
是 **B2.9**：冻结并执行一项针对质量及负载位置泛化的学习侧修复，再用全新、
不重叠的开发病例完整计费验证。开发原型通过前，不进入 B3。
...
15. **B2.9 学习侧质量与泛化修复**：下一独立切片；冻结一项模型侧修复，
    并在全新、不重叠的开发病例上验证。
16. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
    边界，再分成独立可审查的切片实现和审计数据。
17. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
18. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
    B4 通过后执行。
19. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
