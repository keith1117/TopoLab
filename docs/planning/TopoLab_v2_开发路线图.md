...
数据 Gate 未通过，B2.3 另行版本化的 240 次预算探针已通过；B2 的 learned 原型 Gate 仍未通过；
...
这只是抽样数据可行性结果，不等于完整 522 个标签的数据 Gate。下一独立切片
**B2.4** 将在新预算下冻结并生成完整开发标签目录，保留所有失败与资源开销；
只有完整数据 Gate 通过，才进入 B2.5 原型拟合。
...
10. **B2.4 开发标签**：下一独立切片；版本化完整 522 案目录，从干净提交生成
    全部所需标签，并执行完整数据与资源 Gate。
11. **B2.5 原型拟合**：仅在 B2.4 通过后比较固定的 control/candidate，保留
