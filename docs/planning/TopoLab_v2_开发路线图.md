...
状态：**v2 旗舰项目交付路线图；Gate A1、A3 已通过；B2 试点完成但 Gate
未通过；ML 加速是必需的交付 Gate，并非可选增强。** 已发布的 `v1.0.0`
及 M1/M2 的负面结论仍是历史证据。
...
小网格和有效大网格的平均时间比分别为 1.64 和 1.77。因此，两个规模上的质量
可行工作负载与可用 learned 原型尚未建立。下一独立切片 **B2.1** 将针对大网格、
高体积分数收敛问题：先冻结数值验收 case，再检验一项明确的 OC 或终止策略修正；
若数值语义改变，更新求解器与 case identity，并在相同设置下重跑全部比较方法。
若目标工作负载仍不可行，需另建版本化试点。之后另以独立切片检验针对质量问题的
learned 原型，再进入 B3。本试点只涵盖固定支撑族，不能据此宣称其他支撑条件。
...
7. **B2.1 大网格/高体积分数收敛干预**：下一独立切片；先建立质量可行的比较
   分母，再独立检验改进后的 learned 原型，之后才考虑 B3。
8. **B3 新版本 ML 契约和数据 Gate**：先冻结新的 final 边界，再分成独立可审查
   的切片实现和审计数据。
9. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
10. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
   B4 通过后执行。
11. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
