...
但 B2 的 learned 原型 Gate 仍未通过；ML 加速是必需的交付 Gate，并非可选增强。**
...
和 16/30，计入回退后的平均时间比为 1.56 和 1.72。下一独立切片 **B2.2**
将冻结一个仅用于开发的质量对齐 learned 原型计划与数据可行性 Gate，直接针对
未见过的 `z` 方向以及 warm-start 最终质量吸引域问题。拟合前固定 control/
candidate、暴露清单、label 与训练预算、包含回退的验证规则。在开发用原型证明
同等质量的端到端收益之前，不进入 B3 的新 final cohort 预注册。
...
8. **B2.2 有界 learned 原型计划与数据可行性 Gate**：下一独立切片；为方向覆盖
   和最终质量吸引域问题固定开发用 control/candidate 及数据边界。
9. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
   边界，再分成独立可审查
   的切片实现和审计数据。
10. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
11. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
   B4 通过后执行。
12. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
