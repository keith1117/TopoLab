...
下一独立切片 **B2.5** 将使用这些已验证标签，对预先固定的对照组和候选组
进行拟合与筛选，使用同为 240 次预算的 uniform 参考，并完整计入回退成本。
在开发用原型证明同等质量的端到端收益之前，不进入 B3 的新 final cohort 预注册。
...
10. **B2.4 开发标签**：已完成；全部 522 个版本化标签及其工件、数值、病例
    数量和资源审计均已通过。
11. **B2.5 原型拟合**：下一独立切片；比较固定的 control/candidate，保留
    所有病例与失败尝试，计入推理、细化及回退的全部开销。
12. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
   边界，再分成独立可审查
   的切片实现和审计数据。
13. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
14. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
   B4 通过后执行。
15. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
