...
B2.5–B2.9 的固定 learned 原型可行性 Gate 均未通过；
...
- [B2.8 y 方向起点混合与新开发集筛选](../validation/b2_8_basin.md)
- [B2.9 向量点载荷条件化与新开发集筛选](../validation/b2_9_vector_load.md)
...
0.933、0.920，两者的 y 方向均超过 1.0。这促成了 B2.8 固定的 y 方向
起点干预。开发原型通过前，不注册 B3 最终病例。

**B2.8 已完成，开发可行性 Gate 未通过：**
[固定的 y 方向起点混合干预](../validation/b2_8_basin.md)复用三个已审计的 B2.7
模型，在全新 12 个开发病例上完成 132 条方法结果。混合起点确实改变了 y 方向
求解轨迹，却没有减少三个种子的 2/3/4 次失败；没有种子达到双网格与分方向的
完整计费耗时标准。在这批新负载位置和体积分数病例上，未改动的 z 方向预测也
明显偏慢，而不可部署的真实轨迹 oracle 仍显示条件性加速空间。下一独立切片
是 **B2.9**：执行一项针对质量及负载位置泛化的学习侧修复，再用全新、
不重叠的开发病例完整计费验证。

**B2.9 已完成，开发可行性 Gate 未通过：**
[向量点载荷条件化干预](../validation/b2_9_vector_load.md)复用 480 个经审计的
B2.6 目标，完成三次固定拟合，并保存全新 12 个病例的 132 条结果。种子 29、
43 均无回退，小/大网格完整计费平均耗时比分别为 0.613/0.871 和
0.639/0.882；但其大网格 y 方向平均为 1.032、1.080，超过冻结的 1.0
上限。在体积分数 0.5625 的大网格 y 病例中，通过质量检查的起点仍比 uniform
需要更多求解更新。下一独立切片为 **B2.10**：冻结一项针对该成本缺口的质量/
耗时对齐干预，并在全新、不重叠的开发病例上筛查。开发原型通过前，不进入 B3。
...
14. **B2.8 质量/求解轨迹区域干预**：已完成；固定的 y 方向起点混合改变了
    求解轨迹，但没有减少失败，也没有通过双网格与分方向完整计费 Gate。
15. **B2.9 向量点载荷条件化**：已完成；两个种子无回退并满足双网格平均耗时
    上限，但均超过大网格 y 方向上限，因此固定可行性 Gate 未通过。
16. **B2.10 质量/耗时对齐干预**：下一独立切片；针对已测得的大网格 y 方向
    refinement 成本冻结一项修复，在全新、不重叠的病例上完整计费筛查。
17. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
    边界，再分成独立可审查的切片实现和审计数据。
18. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
19. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
    B4 通过后执行。
20. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
