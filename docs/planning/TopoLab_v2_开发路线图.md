...
状态：**v2 旗舰项目交付路线图；Gate A1、A3 已通过；B2.1 数值修正通过，
但 B2.2 数据可行性 Gate 未通过，B2 的 learned 原型 Gate 仍未通过；
...
在 120 次更新后仍不收敛。因此没有生成新标签，也没有拟合模型。下一独立切片
**B2.3** 将针对这八个病例诊断并测试一种单独版本化的数值规则或迭代预算修正，
然后重新验证原有 61 个探针。完整数据生成与拟合仍是后续各自独立的切片。
...
9. **B2.3 有界数据可行性修复**：下一独立切片；保留原有 61 个病例分母，
   只版本化一种数值规则或预算修正，重新验证探针后才能生成完整数据或拟合。
10. **B2.4 开发标签与 B2.5 原型拟合**：仅在前述数据 Gate 通过后，以各自独立
    的切片执行，并保留所有病例、开销与失败的模型尝试。
11. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
   边界，再分成独立可审查
   的切片实现和审计数据。
12. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
13. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
   B4 通过后执行。
14. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
