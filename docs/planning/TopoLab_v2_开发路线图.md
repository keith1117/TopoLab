...
状态：**v2 旗舰项目交付路线图；Gate A1、A3 已通过；B2.2 的 120 次预算
数据 Gate 未通过，B2.3 的 240 次预算探针已通过，B2.4 的完整 522 标签开发数据 Gate 已通过，
B2.5–B2.7 的固定 learned 原型可行性 Gate 均未通过；
ML 加速是必需的交付 Gate，并非可选增强。**
已发布的 `v1.0.0` 及 M1/M2 的负面结论仍是历史证据。
...
- [B2.2 原型方案与数据可行性结果](../validation/b2_2_data_feasibility.md)
- [B2.3 预算修复与探针审计](../validation/b2_3_data_feasibility.md)
- [B2.4 完整开发标签审计](../validation/b2_4_development_labels.md)
- [B2.5 固定原型与完整开发筛选](../validation/b2_5_prototype.md)
- [B2.6 中间轨迹目标与新开发集筛选](../validation/b2_6_trajectory.md)
- [B2.7 全域负载表示与新开发集筛选](../validation/b2_7_global_load.md)
...
**A3 已完成：**[Apple arm64 与 Linux x86-64 配对审计](../validation/a3_solver_ordering.md)
确定了较大规模的 SuperLU 排序策略，并保留历史策略作为显式回退。稀疏求解性能
Gate 已通过；小网格冻结的求解路径以及历史 M1/M2 结果未改变。后续 B2 试点的
所有比较方法都使用相同的优化后排序。
...
**B1 已完成：**[仅用于开发的诊断](../validation/b1_failure_diagnosis.md)在原第 120 次
复现了全部 10 个失败；独立的内存中延长运行在第 127–195 次收敛，历史失败结论
保持不变，并明确决定暂不修改求解器。36 个 M2 validation case 与五个固定 M1
seed 的 180 次重放中，46 次未通过质量检查；未见过的 `z` 方向问题更明显，熟悉的
`y` 方向仍有失败。体积投影和初始 compliance 都不能单独保证最终质量。随后
A3 冻结了较大网格的 uniform 求解器排序，B2 对此后的比较分母进行了试点。
...
**B2 已完成，Gate 未通过：**[固定的开发用试点](../validation/b2_workload_pilot.md)
在两个网格规模各测试六个 case。小网格的六个 uniform 参考结果全部通过，
大网格只有三个在冻结的 120 次迭代内收敛。已知自身 uniform 结果的“不可能
oracle”在有效参考结果上显示条件性加速空间；五个未修改的 M1 seed 计入回退后，
小网格和有效大网格的平均时间比分别为 1.64 和 1.77。因此，历史 B2 试点当时
未能建立两个规模上的质量可行比较分母或可用 learned 原型。本试点只涵盖固定
支撑族，不能据此宣称其他支撑条件。

**B2.1 数值 Gate 已通过，learned 原型 Gate 仍未通过：**
[另行版本化的物理状态平台规则及配对审计](../validation/b2_1_convergence.md)保留旧版
求解器作为默认，并为新策略与结果建立独立 identity。六个小网格、六个大网格的
uniform 参考结果都在 120 次内通过预先声明的数值验收；原先三个大网格失败案例
现在也有同版比较分母。新策略下，不可能的 oracle 在两个规模都通过质量检查，
平均计费时间比分别为 0.074 与 0.109；原有五个 M1 seed 的候选通过数为 20/30
和 16/30，计入回退后的平均时间比为 1.56 和 1.72。

**B2.2 数据可行性 Gate 未通过：**[固定的原型方案与探针结果](../validation/b2_2_data_feasibility.md)
确定了相同 CNN 的 design-MSE 对照组与敏感度加权候选组，以及新策略下的 `y/z`
标签、暴露边界和计算预算。61 个固定 uniform 探针只有 53 个通过独立质量检查；
七个旧 M2 的小网格 `y/0.45` 训练病例，以及一个新的大网格 `z/0.20` 病例，
在 120 次更新后仍不收敛。因此没有生成新标签，也没有拟合模型。

**B2.3 有界探针 Gate 已通过：**[版本化的 240 次预算修复](../validation/b2_3_data_feasibility.md)
保留 B2.1 停止规则和全部数值阈值，为病例与结果建立新 identity，并通过全部
**61/61** 个独立质量检查。原七个小网格失败病例在第 125 次停止，新大网格失败
病例在第 170 次停止；原已成功的 53 个病例，其停止步数与最终柔度完全不变。
这只是抽样数据可行性结果，不等于完整 522 个标签的数据 Gate。

**B2.4 完整开发数据 Gate 已通过：**[固定的 522 病例标签生成与审计](../validation/b2_4_development_labels.md)
在两种网格与所有开发分层上获得 **522/522** 个有效标签，失败数为 0。工件
校验和、持久化的 float32 状态、敏感度权重与独立柔度复算全部通过。生成加审计
共耗时 1,149.58 秒、峰值内存 402.5 MiB，低于预定的两小时与 1 GiB 上限。

**B2.5 已完成，learned 原型 Gate 未通过：**[固定的六次拟合与 54 病例开发筛选](../validation/b2_5_prototype.md)
保留全部 486 条方法结果，并对每次失败尝试及重新运行的 uniform 回退完整计费。
表现最好的 learned 种子是未加权对照组 43，在小、大网格上的平均耗时分别为
对应 uniform 的 1.289 和 1.336 倍；全部六个 learned 种子在两个尺度上均超过
1.0。接受后的质量违规为 0，但有 40 次 learned 尝试失败并触发回退。
敏感度加权候选组未解决收敛、柔度质量及细化耗时瓶颈。

**B2.6 已完成，开发可行性 Gate 未通过：**
[固定的中间轨迹目标干预](../validation/b2_6_trajectory.md)采集了全部 480 个目标，
完成三次拟合和新的、互不重叠的 12 病例筛选。完整计入回退后，三个新种子在
小、大网格上均慢于配对 uniform。表现最好的种子 17 平均耗时比分别为 1.486
和 1.701；种子 29、43 的失败尝试更多。不可部署的真实轨迹 oracle 平均耗时比
为 0.490 和 0.748，说明存在条件性空间，但并不构成 learned 成功证据。
这促成了 B2.7 的单独版本化输入表示修复。在开发用原型证明同等质量的
端到端收益之前，不进入 B3 的新 final cohort 预注册。

**B2.7 已完成，开发可行性 Gate 未通过：**
[固定的全域负载表示干预](../validation/b2_7_global_load.md)复用了 B2.6 的全部
480 个审计目标，完成三次拟合和全新、互不重叠的 12 病例、132 条结果筛选。
新表示让种子 17、29 相比 B2.6 明显改善，但没有种子同时达到双网格与分方向
耗时标准：种子 17 的小、大网格平均耗时比为 0.842、1.049；种子 29 为
0.933、0.920，两者的 y 方向均超过 1.0。下一独立切片是 **B2.8**：针对
实测 y 方向质量失败与接受后细化仍慢的问题，冻结一项与质量/求解轨迹区域
对齐的修复，再用全新开发病例完整计费验证。开发原型通过前，不注册 B3
最终病例。
...
4. **B1 数值收敛与 learned 质量问题溯因**：已完成；未进行新训练、final
   证据评估或求解器修改。
5. **影响基线的 A3 求解器性能工程**：已完成；通过跨平台配对验证，冻结较大
   网格的 uniform 排序策略。
6. **B2 工作负载/网格加速空间试点**：已完成，Gate 未通过。oracle 的条件性
   加速空间尚不能解决大网格参考结果不收敛和现有 learned 方法计费后更慢的问题。
7. **B2.1 大网格/高体积分数收敛干预**：已完成；单独版本化的可选规则让
   12 个 uniform 参考结果全部通过，但旧 learned 面板完整计费后仍更慢。
8. **B2.2 有界 learned 原型计划与数据可行性 Gate**：已完成；固定了 control/
   candidate 与 522 个开发病例，但 61 个探针只有 53 个通过，因此未开始拟合。
9. **B2.3 有界数据可行性修复**：已完成；单独版本化的 240 次预算让固定 61 案
   全部通过，同时保留先前 53 个成功结果。
10. **B2.4 开发标签**：已完成；全部 522 个版本化标签及其工件、数值、病例
    数量和资源审计均已通过。
11. **B2.5 原型拟合与筛选**：已完成；固定的六次拟合与全部 54 个验证病例均已
    审计，但完整计入回退后，learned 原型 Gate 未通过。
12. **B2.6 中间轨迹目标**：已完成；480 个目标、三次拟合和全部 12 个新筛选
    病例均已审计，但没有种子通过完整计入回退的双网格可行性 Gate。
13. **B2.7 全域负载表示修复**：已完成；三次拟合与全新 12 病例筛选让种子
    17、29 相比 B2.6 改善，但双网格与分方向可行性 Gate 未通过。
14. **B2.8 质量/求解轨迹区域干预**：下一独立切片；针对实测的 y 方向质量
    失败和接受后细化仍慢，冻结一项修复并用全新、不重叠的开发病例验证。
15. **B3 新版本 ML 契约和数据 Gate**：开发用原型证明收益后，先冻结新的 final
    边界，再分成独立可审查的切片实现和审计数据。
16. **A2.1–A2.2 进程隔离**：在昂贵的最终 ML 评估之前，以独立 PR 完成。
17. **B4 拟合、可靠性策略与冻结**仅在 B3 通过后执行；**B5 final 评估**仅在
    B4 通过后执行。
18. **A2.3–A2.4 与 A4**按平台要求及资源情况推进。
