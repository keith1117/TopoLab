...
Chinese reading companion: [美研申请匹配度评估](./TopoLab_美研申请匹配度评估.md).
Related documents: [Development timeline and resources](./TopoLab_development_timeline_and_resources.md) · [Numerical Core reimplementation strategy](./TopoLab_reimplementation_strategy.md)

Current v2 delivery rule: the ratings and `v1.0.0` title below are historical
positioning assessments. The active
[v2 roadmap](./TopoLab_v2_development_roadmap.md) requires a positive,
...
| ML negative or unstable | Platform with rigorous warm-start evaluation; full v2 flagship still in development | accelerated optimization, full flagship delivered |
