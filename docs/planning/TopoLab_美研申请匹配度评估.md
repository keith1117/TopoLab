...
English version: [Graduate Admissions Fit Assessment](./TopoLab_admissions_fit_assessment.md)

阅读提示：本文保留早期申请定位评估；完整 v2 旗舰项目的交付必须通过
[v2 路线图](./TopoLab_v2_开发路线图.md)规定的 ML 端到端加速 Gate。

相关文档：[开发周期与资源评估](./TopoLab_开发周期与资源评估.md) · [Numerical Core 重写策略](./TopoLab_数值核心重写策略.md)
