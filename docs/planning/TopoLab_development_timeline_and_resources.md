...
生成器版本、数据分割和 baseline。前两个 M0 切片已在
`docs/ml_experiment_contract.md` 冻结这些契约、加入 typed case identity，并实现
确定性 case encoding 与 filtered-volume projection；尚未实现数据生成器或
manifest，因此 M0 gate 仍未通过，在通过前不能开始模型训练。
