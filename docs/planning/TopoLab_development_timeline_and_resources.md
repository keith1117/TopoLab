...
同状态 compliance、physical volume fraction 和 density change 收敛曲线也已完成。
下一独立切片是前端运行取消控制；之后进入稀疏求解性能与内存 benchmark。
ML 仍需先通过 M0，冻结 case schema、生成器版本、数据分割和 baseline，不能
直接跳到模型训练。
