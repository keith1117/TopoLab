...
## Additive v1.0.0 persistence work

Optional SQLite persistence and explicit process-restart semantics are additive work
after the frozen v0.3.0 contract. Their stable v1.0.0 contract is documented in
`docs/run_persistence.md`; the default remains in-memory unless a database path or
store is supplied. The same additive contract defines UTC run timestamps and stable
