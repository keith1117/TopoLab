"""B2.9 frozen vector-load fitting and disjoint development screen."""
...
from topolab.b2_9_vector_load import (
    ENCODING_VERSION,
    MODEL_VERSION,
...
PLAN_VERSION = "topolab.b2_9.vector_load.v1"
EXPECTED_PLAN_SHA256 = "b7703931dfe2eadbe9300506f288787e44b011e003c4e6fef67cd73ee9000138"
B27_FIT_INDEX_SHA256 = "36158e03a476fdff183d8c72cf0e7143700e7bac3ccc531d7eda485ec390a631"
VOLUMES = (0.2625, 0.4125, 0.5625)
FIT_CAP = 7_200.0
SCREEN_CAP = 14_400.0
...
def cohorts() -> tuple[dict[str, tuple[ExperimentCase, ...]], tuple[ExperimentCase, ...]]:
    """Keep the B2.6 fitting population and freeze one new complete screen."""

    old, b27_screen = b27_cohorts()
    _, b28_screen = b28_cohorts()
    exposed = {
        case.case_id for cases in old.values() for case in cases
    } | {case.case_id for case in (*b27_screen, *b28_screen)}
    historical_volumes = {
    ...
    if set(VOLUMES).intersection(historical_volumes):
        raise ValueError("B2.9 volumes overlap historical development or final designs")
    selected: list[ExperimentCase] = []
    for volume in VOLUMES:
        for direction in ("y", "z"):
            small = _small_case(volume, direction, 1, 2)
    ...
    if (
        ...
        raise ValueError("B2.9 screen identity or strata overlap")
    return old, screen
...
def _index(root: Path, name: str, context: dict[str, Any], maximum: int) -> dict[str, Any]:
    ...
    if index.get("context") != context or len(index.get("rows", [])) > maximum:
        raise ValueError(f"B2.9 {name} context or population differs")
    return index
...
def _read_model(root: Path, row: dict[str, Any], context: dict[str, Any]) -> VectorLoadCNN:
    ...
    if sha256(contents) != row["checkpoint_sha256"] or len(contents) != row["checkpoint_bytes"]:
        raise ValueError("B2.9 checkpoint checksum or size differs")
    ...
    if metadata != {
        "version": "topolab.b2_9.checkpoint.v1", "context": context,
        "seed": seed, "selected_epoch": row["selected_epoch"],
    }:
        raise ValueError("B2.9 checkpoint provenance differs")
    ...
    if set(state) != set(expected) or any(
        ...
        raise ValueError("B2.9 checkpoint tensor differs")
...
def _read_selection(root: Path, row: dict[str, Any], context: dict[str, Any]) -> None:
    ...
    if sha256(contents) != row["selection_sha256"] or len(contents) != row["selection_bytes"]:
        raise ValueError("B2.9 selection checksum or size differs")
    selection = json.loads(contents)
    if (
        canonical_bytes(selection) != contents
        or selection["version"] != "topolab.b2_9.selection.v1"
    ...
    if first != row["selected_epoch"] or minimum != row["selected_validation_loss"]:
        raise ValueError("B2.9 selection is not the earliest minimum")
    if len(history) < 200 and len(history) - first != 25:
        raise ValueError("B2.9 selection violates patience")
...
def fit_models(
    ...
    for row, seed in zip(index["rows"], SEEDS, strict=False):
        if row["seed"] != seed:
            raise ValueError("B2.9 fit order differs")
        _read_selection(root, row, context)
    started = perf_counter()
    prior = index["elapsed_seconds"]
    if len(index["rows"]) < 3:
        train = _samples(
            b26_root, old, targets, "train", encoder=encode_vector_load_case,
            input_channels=13,
        )
        validation = _samples(
            b26_root, old, targets, "validation", encoder=encode_vector_load_case,
            input_channels=13,
        )
        for position in range(len(index["rows"]), 3):
            if prior + perf_counter() - started > FIT_CAP:
                raise ValueError("B2.9 fit wall budget exhausted")
            seed = SEEDS[position]
            ...
            selection = {
                "version": "topolab.b2_9.selection.v1", "context": context,
            ...
            print(f"B2.9 fit {position + 1}/3 seed={seed}", file=sys.stderr, flush=True)
    index["elapsed_seconds"] = prior + perf_counter() - started
    index["peak_rss_bytes"] = max(index["peak_rss_bytes"], _peak_rss())
    atomic_write(root / "fit_index.json", canonical_bytes(index))
    if len(index["rows"]) != 3 or index["elapsed_seconds"] > FIT_CAP or (
        ...
        raise ValueError("B2.9 complete fit resource Gate failed")
    return index


def screen_models(
    root: Path, b26_root: Path, b27_root: Path, b24_root: Path,
    b25_root: Path, screen: tuple[ExperimentCase, ...], context: dict[str, Any]
) -> dict[str, Any]:
    ...
    if len(fits["rows"]) != 3 or fits["elapsed_seconds"] > FIT_CAP or (
        ...
        raise ValueError("B2.9 screen requires complete audited fits")
    for row, seed in zip(fits["rows"], SEEDS, strict=True):
        if row["seed"] != seed:
            raise ValueError("B2.9 model order differs")
        _read_selection(root, row, context)
    screen_context = {
    ...
    for position, row in enumerate(index["rows"]):
        if row["case"]["case_id"] != screen[position].case_id or [
            ...
            raise ValueError("B2.9 persisted screen row differs")
    started = perf_counter()
    prior = index["elapsed_seconds"]
    if len(index["rows"]) < 12:
        load_started = perf_counter()
        new_models = {row["seed"]: _read_model(root, row, context) for row in fits["rows"]}
        if sha256((b27_root / "fit_index.json").read_bytes()) != B27_FIT_INDEX_SHA256:
            raise ValueError("B2.7 fixed fit index differs")
        old_fits = json.loads((b27_root / "fit_index.json").read_bytes())
        old_models = {}
        for row in old_fits["rows"]:
            read_b27_selection(b27_root, row, old_fits["context"])
            old_models[row["seed"]] = read_b27_model(b27_root, row, old_fits["context"])
        ...
        for position in range(len(index["rows"]), 12):
            if prior + perf_counter() - started > SCREEN_CAP:
                raise ValueError("B2.9 screen wall budget exhausted")
            ...
            print(f"B2.9 screen {position + 1}/12", file=sys.stderr, flush=True)
    index["elapsed_seconds"] = prior + perf_counter() - started
    index["peak_rss_bytes"] = max(index["peak_rss_bytes"], _peak_rss())
    atomic_write(root / "screen_index.json", canonical_bytes(index))
    return _screen_summary(
        root, index, learned_prefix="vector_",
        summary_version="topolab.b2_9.screen_summary.v1",
    )
...
def main() -> int:
    ...
    if plan["plan_sha256"] != EXPECTED_PLAN_SHA256:
        raise ValueError("B2.9 plan differs from frozen identity")
    ...
    if len({root, b26_root, b27_root}) != 3:
        raise ValueError("B2.9 output and source roots must differ")
    context = {
    ...
    summary = screen_models(
        root, b26_root, b27_root, args.b24_root.resolve(),
        args.b25_root.resolve(), screen, context,
    )
